...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // If redirected, page will navigate away — no need to setBusy(false)
    } catch {
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <button
              ...
              disabled={busy}
              className="w-full flex items-center justify-center gap-3 rounded-lg border border-border bg-card px-4 py-3 text-sm font-medium text-foreground shadow-sm transition-colors hover:bg-muted disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {busy ? (
